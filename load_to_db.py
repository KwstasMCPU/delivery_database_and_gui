...
path = 'cleaned_data/'
csv_list = os.listdir('cleaned_data/') # saving the .csv files name into a list, in order to perfrom iteration on it in the make_the_dataframes()

def make_the_dataframes():
    """
    this function reads the csv files located in the 'cleaned_data' relative path, and turns them to a dataframe.
    """
...
def load_dataframe_to_db(df, table, DATABASE_NAME='delivery.db', chunk=100):
    ...
    Parameters:
        ...
        chunk (int): Specify the number of rows in each batch to be written at a time. 
                     Is assign to the chunksize parameter of the to_sql() function, which by default tries to write all rows at once. (default 100)
    """
    conn = sqlite3.connect(DATABASE_NAME)
    df.to_sql(table, conn,
                if_exists='append', # we used append since the tables, although empty, they already exist. If replaced was used, the tables and their variable types would had been replaced.
                index = False, # do not load the dataframe index as an sqlite db column, rowid is created by default as well
                chunksize = chunk)
    conn.commit()
    conn.close()
...
def show_table_rows(table_name, DATABASE_NAME='delivery.db', rows = 100):
    """
    This function makes an SQL SELECT query and prints the results.
    Parameters:
...
def delete_table(table_name, DATABASE_NAME = 'delivery.db'):
    """
    This functions deletes a table from the database
    Parameters:
...
# create the panda dataframes
customer_clean_df, locations_clean_df, orders_clean_df, vendors_clean_df = make_the_dataframes()

# UNCOMMENT BELOW CODE IF YOU WISH TO LOAD THE DATAFRAMES IN THE DATABASE
